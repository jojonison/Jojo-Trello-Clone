...
<template>
  <main class="bg-blue-400 border-[6px] border-blue-950 rounded-xl shadow-lg m-2">
    <div v-if="props.selectedProject === undefined">
      Please select a project first
    </div>
    <div v-for="task in unfinishedTasks" :key="task.id" class="bg-blue-200 border-[2px] border-blue-950 p-2">
      {{ task.title }}
      {{ task.description }}
    </div>
  </main>
