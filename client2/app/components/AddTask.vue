  ...
  $api.post(`/create-task`, {
    title: state.value.title, description: state.value.description, project_id: props.selectedProject?.id
      ...
      .then((response: AxiosResponse) => {
        toast.add({ title: 'Success', description: 'Task has been added', color: 'success' })
...
<template>
  <main>
    <UForm :state="state" @submit="addTask">
      ...
      <UButton type="submit" color="neutral" class="border-[2px] border-blue-950 hover:bg-blue-400 bg-blue-300">Add Task!</UButton>
    </UForm>
  </main>
</template>